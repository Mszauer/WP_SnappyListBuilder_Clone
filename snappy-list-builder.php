...
/*
Plugin Name: Snappy List Builder Clone
Plugin URI: http://wordpressplugincourse.com/plugins/snappy-list-builder
Description: A copy of the original Snappy List Builder cloned for learning purposes: The ultimate email list building plugin for WordPress. Capture new subscribers. Reward subscribers with a custom download upon opt-in. Build unlimited lists. Import and export subscribers easily with .csv
Version: 1.0.1
Author: Martin Szauer
Author URI: http://www.martinszauer.com
License: GPL2
...
// 2.2 returns a html string for a email capture form
...
// 2.3 displays a form for managing the users list subscriptions
...
// 2.4 displays subscription opt-in confirmation text and link to manage sunscriptions
...
// 2.5 returns a message if the download link has expired or is invalid
// [slb_download_reward]
...
// 3.5 registers custom plugin admin menus
...
// 5.2 creates a new subscriber or updates and existing one
...
// 5.3 adds list to subscribers subscriptions
...
// 5.4 removes one or more subscriptions from a subscriber and notifies them via email
...
// 5.5 removes a single subscription from a subscriber
...
// 5.6 sends a unqiue customized email to a subscriber
...
// 5.7 adds subcription to database and emails subscriber confirmation email
...
// 5.8 creates custom tables for our plugin
...
// 5.10 adds new reward links to the database
...
// 5.11 triggers a download of the reward file
...
// 5.12 increases reward link download count by one
...
// 5.13 generates a .csv file of subscribers data
...
// 5.14 this function retrieves a csv file from the server and parses the data into a php array
...
// 5.15 adds a new admin notice to array $slb_notices
...
// 5.16 imports new subscribers from our import admin page
...
// 5.17 checks the current version of wordpress against supported versions and displays a admin notice if it is not supported
...
// 5.18 runs functions for plugin deactivation
...
// 5.19 removes our custom database tabels
...
// 5.20 removes plugin related custom post type post data
...
// 5.21 removes any custom options from the database 
...
// 6.2 retrieves a subscriber_id from an email address
...
// 6.3 returns an array of list_id's
...
//6.5 gets the unique act field key from the field name
...
// 6.6 returns an array of subscriber data including subscriptions
...
// 6.7 returns html for a page selector
...
// 6.8 returns default option values as an associative array
...
// 6.9 returns the requested page option value or it's default
...
// 6.10 get's the current options and returns values in associative array
...
// 6.11 generates an html form for managing subscriptions
...
// 6.12 returns an array of email template data IF the template exists
...
// 6.13 validates whether the post object exists and that it's a validate post_type
...
// 6.14 validates whether the post object exists and that it's a validate post_type
...
// 6.15 returns a unique link for managing a particular users subscriptions
...
// 6.16 returns the appropriate character for the begining of a querystring
...
// 6.17 returns a unique link for opting into an email list
...
// 6.19 returns false if list has no reward or returns the object containing file and title if it does
...
// 6.20 returns a unique link for downloading a reward file
...
// 6.22 returns false if list has no reward or returns the object containing file and title if it does
...
// 6.23 returns an array of subscriber_id's
...
// 6.24 returns the amount of subscribers in this list
...
// 6.25 returns a unique link for downloading a subscribers csv
...
// 6.26 this function reads a csv file and converts the contents into a php array
...
// 6.27 returns html formatted for WP admin notices
...
// 6.28 get's an array of plugin option data (group and settings) so as to save it all in one place
...
// 8.2 import subscribers admin page
